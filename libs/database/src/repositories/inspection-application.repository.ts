...
export class InspectionApplicationRepository {
  ...
  async findApplicationByTargetAndSchedule(
    ...
    return await this.databaseService.inspectionApplication
      ...
      .then(async (app) => {
        const [decryptedApp, inspectionTargetInfo] = await Promise.all([
  ...
  async updateApplicationStatusInTx(
    ...
    return await tx.inspectionApplication
      ...
      .then(async (app) => {
        const [decryptedApp, user] = await Promise.all([
          this.encryptionService.decryptApplication(app),
  ...
  async updateDocumentActiveStatus(
    ...
    return await this.databaseService.inspectionApplication
      ...
      .then((app) => this.encryptionService.decryptApplication(app))
      .catch((error) => {
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
          if (error.code === 'P2025') {
            this.logger.debug(
  ...
  async findApplicationByUuid(uuid: string): Promise<ApplicationInfo> {
    return await this.databaseService.inspectionApplication
      ...
      .then(async (app) => {
        const [decryptedApp, user, inspector, inspectionTargetInfo] =
          await Promise.all([
            this.encryptionService.decryptApplication(app),
            this.encryptionService.decryptUser(app.user),
  ...
  async findApplicationDocumentsByScheduleUuid(
    ...
    return await this.databaseService.inspectionApplication
      ...
      .then((apps) =>
        Promise.all(
          apps.map((app) => this.encryptionService.decryptApplication(app)),
        ),
      )
      .catch((error) => {
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
